...
final class ObserveStateSpec
  ...
  test("NIFS normal observation") {
    ...
    assertEquals(observeErrorCount.get(), 0)
    assertEquals(observePauseCount.get(), 0)
    assertEquals(observeSuccessCount.get(), 1)
  }
  ...
  test("GMOS paused observation") {
    ...
    assertEquals(state, CaCommandMonitor.State.PAUSE)

    // Check that listener was called
    assertEquals(observeErrorCount.get(), 0)
    assertEquals(observePauseCount.get(), 1)
    assertEquals(observeSuccessCount.get(), 0)
  ...
  test("GMOS observation with an error case 1") {
    ...
    // Check that listener was called
    assertEquals(observeErrorCount.get(), 1)
    assertEquals(observePauseCount.get(), 0)
    assertEquals(observeSuccessCount.get(), 0)
  }
...
trait ChannelsFactory extends MockFactoryBase {
  type ExpectationException = RuntimeException
  def newExpectationException(message: String, methodName: Option[Symbol]) =
    new RuntimeException(s"Mock expectation in $methodName failed: $message")
